...
//! rewritten in zig
pub const OffsetAllocator = @This();
...
pub const Allocation = struct {
    ...
    pub const invalid: Allocation = .{
        .offset = std.math.maxInt(u64),
        .chunk = std.math.maxInt(u64),
        .size = std.math.maxInt(u64),
    };

    pub fn isInvalid(self: *const Allocation) bool {
        return self.size == std.math.maxInt(u64) and
            self.offset == std.math.maxInt(u64) and
            self.chunk == std.math.maxInt(u64);
    }
};
...
pub fn init(allocator: std.mem.Allocator, size: u32, max_allocations: ?u32) std.mem.Allocator.Error!OffsetAllocator {
    ...
    // first allocator passed in is a no-op, second one is used for allocations
    try self.reset(allocator, allocator);
    return self;
}

/// passed allocator must be the same as used in init, but reallocator can be different
pub fn reset(self: *OffsetAllocator, allocator: std.mem.Allocator, reallocator: std.mem.Allocator) std.mem.Allocator.Error!void {
    self.free_storage = 0;
    ...
    self.nodes = try reallocator.alloc(Node, self.max_allocations);
    self.free_nodes = try reallocator.alloc(NodeIndex, self.max_allocations);
...
/// passed allocator must be the same as used in init
pub fn deinit(self: *OffsetAllocator, allocator: std.mem.Allocator) void {
    if (self.nodes) |nodes| {
        allocator.free(nodes);
        self.nodes = null;
    }
    if (self.free_nodes) |free_nodes| {
        allocator.free(free_nodes);
...
test "basic" {
    var allocator: OffsetAllocator = try .init(std.testing.allocator, 1024 * 1024 * 256, null);
