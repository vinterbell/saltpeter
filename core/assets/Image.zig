...
pub fn createEmpty(width: u32, height: u32, num_components: u32, args: struct {
    ...
    const data = try std.heap.raw_c_allocator.alloc(u8, size);
    @memset(data, 0);
...
pub fn resize(image: *const Image, new_width: u32, new_height: u32) Image {
    ...
    const new_data = try std.heap.raw_c_allocator.alloc(u8, new_size);
...
pub fn writeToFile(
    ...
    if (result == 0) {
        return WriteError.CouldNotWriteImage;
    }
}
...
pub fn deinit(image: *Image) void {
    std.heap.raw_c_allocator.free(image.data);
    image.* = undefined;
}
...
test "stbi image write and load file" {
    ...
    try img.writeToFile("test_img.png", WriteFormat.png);
